Feature: To check exception handlers are thrown when missing/malformed data is sent to the API as a PUT request

    Scenario: a client makes a put request to the add cars/admin endpoint with incorrect data and receives a 400 status and a body of "Incorrect car data provided"
    ...
    Scenario: the client sends a malformed Json request to the /cars/admin endpoint and the error is caught by our global exception handler.
        When the client sends a "PUT" request to "/cars/admin/put" endpoint with a malformed json list
        Then the response should have a status code of 400
        And the response body should have the key '{"Description":"Incorrect car data provided"}'
